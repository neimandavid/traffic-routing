...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        for i in range(len(ids)):
            #traci.vehicle.setRouteID(ids[i], getShortestRoute(["SLL0", "SLR0", "SRR0"]))
            #traci.vehicle.setRouteID(ids[i], getShortestRoute(["SLR0", "SRR0"]))
            traci.vehicle.setRouteID(ids[i], getShortestRoute(["SLL0", "SLR0", "SRR0"]))
        ...
        for i in range(len(ids)):
            ...
            #traci.vehicle.setRouteID(ids[i], getShortestRoute(["LR"]))
            traci.vehicle.setRouteID(ids[i], getShortestRoute(["LL"]))

        ids = traci.multientryexit.getLastStepVehicleIDs("RerouterSR")
        for i in range(len(ids)):
            traci.vehicle.setRouteID(ids[i], getShortestRoute(["SRR"]))

        ids = traci.multientryexit.getLastStepVehicleIDs("RerouterR")
        for i in range(len(ids)):
            traci.vehicle.setRouteID(ids[i], getShortestRoute(["RR"]))
