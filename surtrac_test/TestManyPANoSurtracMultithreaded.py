import os
import sys
import runnerQueueSplit11ThreadedNoSurtracMultithreaded
import pickle
import statistics
...
try:
    with open("delaydata_NoSurtracMultithreaded_" + sys.argv[1] + ".pickle", 'rb') as handle:
        data = pickle.load(handle)
except:
    data = dict()
print(data)

for p in [0.01, 0.05, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99]*3: #[0.01, 0.05, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99]*3: #[0.01, 0.05, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99]*1:
    print(p)
    if not p in data:
        data[p] = dict()
        data[p]["All"] = []
    ...
    for i in range(0, nIters):
        print(i)
        newdata = runnerQueueSplit11ThreadedNoSurtracMultithreaded.main(sys.argv[1], p, False)
        data[p]["All"].append(newdata[0])
        data[p]["Adopters"].append(newdata[1])
        data[p]["Non-Adopters"].append(newdata[2])
        with open("delaydata_NoSurtracMultithreaded_" + sys.argv[1] + ".pickle", 'wb') as handle:
        ...
        plt.savefig("Plots/DelaysNoSurtracMultithreaded" + sys.argv[1].split(".")[0] +".png")
        plt.close()
