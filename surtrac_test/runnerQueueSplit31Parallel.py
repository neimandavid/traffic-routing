...
def generate_additionalfile(sumoconfig, networkfile):
    ...
    if useLibsumo:
        traci.start([checkBinary('sumo'), "-c", sumoconfig,
                                    ...
                                    "--xml-validation", "never", "--nowarnings", "--quit-on-end"])
    else:
        try:
            traci.start([checkBinary('sumo'), "-c", sumoconfig,
                                    "--start", "--no-step-log", "true",
                                    "--xml-validation", "never", "--nowarnings", "--quit-on-end"], label="setup")
        except:
...
def main(sumoconfigin, pSmart, verbose = True, useLastRNGState = False, appendTrainingDataIn = False, predDiscountIn=predDiscount):
    ...
    if useLibsumo:
        traci.load(["-c", sumoconfig,
                                ...
                                "--no-step-log", "true",
                                "--log", "LOGFILE", "--xml-validation", "never", "--start", "--nowarnings", "--quit-on-end"])
    else:
        try:
            if noGUI:
                traci.start([checkBinary('sumo'), "-c", sumoconfig,
                                        "--additional-files", "additional_autogen.xml",
                                        "--no-step-log", "true",
                                        "--log", "LOGFILE", "--xml-validation", "never", "--start", "--nowarnings", "--quit-on-end"], label="main")
            else:
