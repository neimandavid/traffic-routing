...
#QueueSplit11Threaded doesn't update routes until all vehicles have been rerouted in each main simulation timestep
#QueueSplit11NotThreaded emulates that behavior in a single threaded way for testing
...
def reroute(rerouters, network, simtime, rerouteAuto=True):
    ...
    if doAstar:
        ...
        for vehicle in tempnewroutes:
            newroute = tempnewroutes[vehicle]
...
def QueueReroute(detector, network, simtime, rerouteAuto=True):
    ...
    for vehicle in ids:
        ...
        if rerouteAuto and isSmart[vehicle]:
            ...
            tempnewroutes[vehicle] = newroute
    if rerouteAuto:
        oldids[detector] = ids
