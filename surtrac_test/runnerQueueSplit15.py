...
multithreadRouting = True
multithreadSurtrac = True
reuseSurtrac = True #Does Surtrac computations in a separate thread, shared between all vehicles doing routing. Keep this true unless we need everything single-threaded (ex: for debugging)
debugMode = True #Enables some sanity checks and assert statements that are somewhat slow but helpful for debugging
disableSurtracPred = True #Speeds up code, at the cost of no longer having Surtrac predict future clusters for neighboring intersections
mainSurtracFreq = 5
routingSurtracFreq = 5
recomputeRoutingSurtracFreq = 2
        ...
        for schedule in schedules:
            ...
            for lane in lightlanes[light]:
                ...
                for i in lanephases[lane]:
                    ...
                    #testdirmakespans = np.maximum(directionalMakespans, ast).tolist()
                    ...
                    # #print(testdirmakespans)
                    # #print(directionalMakespans)
                    # assert((testdirmakespans == directionalMakespans))
                    # directionalMakespans = testdirmakespans
...
def reroute(rerouters, network, simtime, remainingDuration):
    ...
    for vehicle in toReroute:
        if multithreadRouting:
            threads[vehicle].join() #This is apparently slow when recomputeRoutingSurtracFreq is big. Not sure why
        ...
        if isSmart[vehicle]:
            ...
            for vehicletemp in vehiclesOnNetwork:
                if not isSmart[vehicletemp]:
                    ...
                    #NEXT TODO: Delete old route generator once I'm convinced nothing breaks (mid-intersection vehicles, etc.)
...
def runClusters(net, routesimtime, mainRemainingDuration, vehicleOfInterest, startlane, loaddata, routes):
    ...
    #Fix routesimtime before we initialize the list of things to check, else we might skip the time we're supposed to check stuff
    #Goal is to ensure that all routing sims do Surtrac at the same times so we can reuse the results
