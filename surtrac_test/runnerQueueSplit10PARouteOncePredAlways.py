...
#Set prediction cutoff to 0 when calling from QueueSim?
...
def doSurtrac(network, simtime, realclusters=None, lightinfo=None, predClusters=None):
    ...
    predictionCutoff = 60 #Overwritten if not in sim
...
def run(network, rerouters, pSmart, verbose = True):
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        for vehicle in traci.simulation.getDepartedIDList(): #Need this after first loop so we know if all new cars are smart before routing anything
            #Route once at start
            if isSmart[vehicle]:
