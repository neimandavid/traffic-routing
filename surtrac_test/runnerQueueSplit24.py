...
simdetectordist = 0 #How far after the start of a road the detectors for reconstructing initial routing sim traffic state are. TODO I suspect 0 just misses cars...
...
def run(network, rerouters, pSmart, verbose = True):
    ...
    while traci.simulation.getMinExpectedNumber() > 0 and (not appendTrainingData or simtime < 5000):
            ...
            if newlane != laneDict[id] and len(newlane) > 0 and  newlane[0] != ":":
                ...
                if newloc != edgeDict[id]: #Moved to a new road
                    if edgeDict[id] in nonExitEdgeDetections:
                        oldEdgeStuff = nonExitEdgeDetections[edgeDict[id]][-1]
                        if len(oldEdgeStuff) > 0:
                            #Loop through, pop oldest from oldEdgeStuff that was in laneDict[id]
                            removedCar = False
                            for ind in range(len(oldEdgeStuff)):
                                if oldEdgeStuff[ind][1] == laneDict[id]:
                                    oldEdgeStuff.pop(ind)
                                    removedCar = True
                                    break
                            if not removedCar: #Apparently there was nothing in the lane we expected to pull from
                                print(nonExitEdgeDetections[edgeDict[id]])
                                print(nonExitLaneDetectors[edgeDict[id]+"_0"])

                                print("Warning: Ran out of cars to remove on lane " + laneDict[id] + ", popping oldest car out of all lanes")
                                oldEdgeStuff.pop(0)
                        else:
                            print(nonExitEdgeDetections[edgeDict[id]])
                            print(nonExitLaneDetectors[edgeDict[id]+"_0"])
                        ...
                        if newloc in nonExitEdgeDetections:
                            ...
                            nonExitEdgeDetections[newloc][0].append((newlane+".0."+str(simtime), newlane, simtime))
        ...
        #NEXT TODO: Redo the oldids smart detector code and see if that fixes stuff
        for lane in nonExitLaneDetectors:
            for ind in range(1, len(nonExitLaneDetectors[lane])): #Skip segment 0, our fake detector code looking for road changes covers that
                ...
                if len(ids) == 0:
                    wasEmpty[detector] = ids #Make sure we don't have a vehicle stopped on the detector repeatedly triggering it. Hopefully we get at least 2 seconds between vehicles, else we might miss some
                else:
                    if detector in wasEmpty: #Else it's been detecting stuff for the entire simulation???
                        
                        edge = lane.split("_")[0]
                        
                        oldEdgeStuff = nonExitEdgeDetections[edge][ind-1]
                        newEdgeStuff = nonExitEdgeDetections[edge][ind]

                        for id in ids:
                            if id in wasEmpty[detector]: #Using this as oldids; TODO change name
                                continue
                            if len(oldEdgeStuff) > 0:
                                oldEdgeStuff.pop(0)
                            else:
                                print("Warning: Ran out of cars to remove on edge " + edgeDict[id] + " section " + str(ind-1))

                            newEdgeStuff.append((lane+"." + str(ind) + "."+str(simtime), lane, simtime))
                        wasEmpty[detector] = ids
...
def loadClustersDetectors(net, simtime, VOI=None):
    global totalLoadCars
    global nVehicles
    #Load locations of cars and current traffic light states into custom data structures
    #If given, VOI is the vehicle triggering the routing call that triggered this, and needs to be unaffected when we add noise
    ...
    for edge in nonExitEdgeDetections: #Assuming exit lanes don't matter since they shouldn't have traffic - this saves us from extra exit detectors at their ends
        ...
        for roadsectionind in reversed(range(len(temp))): #First road section (=closest to start) is listed first, and we want to go backwards from the end, so reverse this
            ...
            for (vehicle, lane, detecttime) in roadsectiondata: #Earliest time (=farthest along road) is listed first, don't reverse this
        ...
        for edge in traci.edge.getIDList():
            ...
            for lanenum in range(traci.edge.getLaneNumber(edge)):
                ...
                if len(net.getEdge(edge).getOutgoing()) > 0:
                    ...
                    for dist in range(0, math.floor(0.9*lengths[lane]), math.ceil(2.1*speeds[edge])): #TODO add to this if we need more detectors, remember to update it both here and below in additionalrouting_autogen
        ...
        for edge in traci.edge.getIDList():
            ...
            for lanenum in range(traci.edge.getLaneNumber(edge)):
                ...
                print('    <inductionLoop id="IL_%s" freq="1" file="outputAuto.xml" lane="%s" pos="-%i" friendlyPos="true" />' \
                      % (lane, lane, detectordist), file=additional)
                if len(net.getEdge(edge).getOutgoing()) > 0:
                    for dist in range(0, math.floor(0.9*lengths[lane]), math.ceil(2.1*speeds[edge])): #TODO add to this if we need more detectors, remember to update it both here and above in additional_autogen
                        name = "ILd_" + lane + "_" + str(dist)
                        print('    <inductionLoop id="%s" freq="1" file="outputAuto.xml" lane="%s" pos="%i" friendlyPos="true" />' \
                        % (name, lane, dist), file=additional)
        print("</additional>", file=additional)
...
def getLeftEdge(startlane, network):
    #Returns the leftmost edge with a connection from startlane, or None if no edge turns left by more than 0.1 radians (about 6 degrees)
    startedge = startlane.split("_")[0]
    leftedge = None
    leftest = 0.1 #Rule out slight lefts and roundoff error
    leftest = -np.inf
...
def loadStateInfoDetectors(prevedge, simtime):
    ...
    for lane in nonExitEdgeDetections: #Assuming exit lanes don't matter since they shouldn't have traffic - this saves us from extra exit detectors at their ends
        temp = nonExitEdgeDetections[lane]
        print(temp)
        print(nonExitEdgeDetections)
        print("WHY ARE WE RUNNING THIS IT'S CLEARLY BAD!!!!!!!!! TODO UPDATE")
    ...
    for edge in nonExitEdgeDetections: #Assuming exit lanes don't matter since they shouldn't have traffic - this saves us from extra exit detectors at their ends
        temp = nonExitEdgeDetections[edge]
        for roadsectionind in reversed(range(len(temp))):
            roadsectiondata = temp[roadsectionind]
            for (vehicle, lane, detecttime) in reversed(roadsectiondata): #Reversed so we go from end of edge to start of edge - first clusters to leave are listed first
                ...
                #lanepos = traci.vehicle.getLanePosition(vehicle)
                newroute = sampleRouteFromTurnData(lane, turndata)
                if vehicle in traci.route.getIDList():
                    traci.route.remove(vehicle) #So we sample different routes for non-adopters in each routing simulation
                traci.route.add(vehicle, newroute) #Using vehicle as the name of the new route, to maximize confusion for future me! (Also so we're guaranteed a unique name for the route)

                traci.vehicle.add(vehicle, vehicle, departLane=lane.split("_")[-1], departPos=lanepos, departSpeed="max")
                if vehicle in isSmart and isSmart[vehicle]: #It's an adopter, use its planned route
                    startroute = currentRoutes[vehicle]
                    startedge = edgeDict[vehicle]
                    startind = startroute.index(startedge)
                    startroute = startroute[startind:]
