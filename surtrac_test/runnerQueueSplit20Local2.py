        ...
        if isSmart[vehicle]:
            try:
                ...
                if multithreadRouting:
                    ...
                    #This is silly, but let's go basically serial for now
                    routingthreads[vehicle].join()
                    [newroute, esttime] = routingresults[vehicle]
                else:
                    [newroute, esttime] = rerouteSUMOGC(vehicle, lane, remainingDuration, mainlastswitchtimes, sumoPredClusters, lightphases, simtime, network, routingresults)
...
def rerouteSUMOGC(startvehicle, startlane, remainingDurationIn, mainlastswitchtimes, sumoPredClusters, lightphases, simtime, network, reroutedata):
    ...
    remainingDuration = pickle.loads(pickle.dumps(remainingDurationIn))
    ...
    while(True):
        ...
        #TODO delete so we actually route
        # reroutedata[startvehicle] = [startroute, -1]
        # return reroutedata[startvehicle]
