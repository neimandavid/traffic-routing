#Randomly generates 3-4 way intersections, 3 lanes per road, lagging lefts on light cycles
...
def intersectionGenerator():
    ...
    for i in range(nRoads):
        nLanes[str(i)] = 3#RIR(0, maxNLanes) #n = somewhere between 0 and maxNLanes lanes on each road
        if i == 3 and isT:
            nLanes[str(i)] = 0
        # if i == 2 and isT:
    ...
    if not isT:
        ...
        lightseqs[0][1] = lightseqs[0][0]
        lightseqs[0][2] = addYellows(cycleArray([0.1, 1, 0, 0], cycleBy)) #Left lane: [g, G, r, r]
        lightseqs[1][0] = addYellows(cycleArray([0, 0, 1, 0], cycleBy))
        lightseqs[1][1] = lightseqs[1][0]
        lightseqs[1][2] = addYellows(cycleArray([0, 0, 0.1, 1], cycleBy)) #Left lane: [r, r, g, G]
        ...
        lightseqs[0][1] = lightseqs[0][0]
        lightseqs[0][2] = addYellows(cycleArray([0.1, 1, 0], cycleBy)) #Left lane: [g, G, r]
        lightseqs[2][0] = addYellows(cycleArray([1, 0, 0], cycleBy)) #Have to stop during opposing left turn arrow
        lightseqs[2][1] = lightseqs[2][0]
        lightseqs[2][2] = lightseqs[2][0] #Road 2 can't turn left, everything's right/straight only
        lightseqs[1][0] = addYellows(cycleArray([0, 1, 1], cycleBy)) #Road 1 right lane can go during the protected left
        lightseqs[1][1] = lightseqs[1][0]
        lightseqs[1][2] = addYellows(cycleArray([0, 0, 1], cycleBy))
