...
import runnerQueueSplit31Parallel as runnerQueueSplit #KEEP THIS UP TO DATE!!! (If training from a network, not just IG)
...
nLossesBeforeReset = 1#1000/batch_size
losses = dict()
epochlosses = dict()    
daggertimes = dict()
...
def main(sumoconfigs):
    ...
    while True:
        ...
        try:
            for light in lights:
                trafficdataset[light] = TrafficDataset("trainingdata/trainingdata_" + sys.argv[1] + ".pickle", light)
        ...
        #Train everything once - note that all losses are likely to spike after new training data comes in
        for light in lights:
            trainLight(light, trafficdataset[light])
        ...
        while True and not superResetTrainingData:
            ...
            trainLight(worstlight, trafficdataset[worstlight])
        ...
        for light in trainingdata:
            ...
            for batch in trainingdata[light]:
                ...
                for linenum in range(batch[0].size(0)):
                    ...
                    if len(batch) > 2:
                        for entry in batch[2][linenum].numpy()[0][0]:
                            sheets[light].cell(row, col, float(entry))
                            col += 1
