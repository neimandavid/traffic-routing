...
#New in QueueSplit11: Compute delay, not just average travel time, for cars
#In QueueSplit11Threading: Multithreaded the Surtrac calls for all the lights, and sampling instead of using turn ratios. Slower and worse delay after 100 timesteps (not sure if more long-route cars finished, which would be good, or if everything slowed down, which would be bad, but it's different, so that's not expected)

#Recent change: In routing, non-priority links that we've already scheduled now block other non-priority links that we haven't yet scheduled
#This seems like it should help, but things are getting worse?
...
def doSurtrac(network, simtime, realclusters=None, lightinfo=None, predClusters=None):
    ...
    bestschedules = dict()
    for light in lights:
        bestschedules[light] = [None]*9
        threads[light] = threading.Thread(target=doOneSurtrac, args=(light, clusters, simtime, lightinfo, inQueueSim, sult, predictionCutoff, toSwitch, catpreds, remainingDuration, bestschedules[light]))
        threads[light].start()

    for light in lights:
        threads[light].join()
        nPhases = len(surtracdata[light]) #Number of phases
        if not bestschedules[light] == [None]*9:
            #remainingDuration = traci.trafficlight.getNextSwitch(light) - simtime
            spentDuration = simtime - lastswitchtimes[light]
            if bestschedules[light][7][0] - spentDuration > 0 and not inQueueSim:
                #Update duration
                traci.trafficlight.setPhaseDuration(light, bestschedules[light][7][0] - spentDuration) #setPhaseDuration sets the remaining duration in the phase
            else:
                ...
                if not inQueueSim:
                    ...
                    if len(bestschedules[light][7]) > 1:
                        #And set the new duration if possible
                        traci.trafficlight.setPhaseDuration(light, bestschedules[light][7][1]) #Update duration if we know it
                        #pass

    return (toSwitch, catpreds, remainingDuration)

def doOneSurtrac(light, clusters, simtime, lightinfo, inQueueSim, sult, predictionCutoff, toSwitch, catpreds, remainingDuration, bestschedule):
                        ...
                        if not cartuple[0] in isSmart or isSmart[cartuple[0]]: #It's possible we call this from QueueSim, at which point we split the vehicle being routed and wouldn't recognize the new names. Anything else should get assigned to isSmart or not on creation
                            ...
                            for nextlaneind in range(lanenums[nextedge]):
                                ...
                                if nextlane in predLanes:
                                    r -= turndata[lane][nextlane] / normprob
                                    if r <= 0:
                                        newPredClusters[nextlane][-1]["cars"].append(cartuple)
                                        newPredClusters[nextlane][-1]["weight"] += 1
                        else:
    ...
    for schedule in schedules:
        if schedule[5] < mindelay:
            ...
            for i in range(len(schedule)):
                bestschedule[i] = schedule[i] #Want changes to persist outside the thread, so change the elements rather than the pointer
...
def runClusters(net, time, vehicleOfInterest, startedge, loaddata):
    ...
    while True:
        ...
        for light in remainingDuration:
            ...
            while remainingDuration[light][0] < 0:
                remainingDuration[light].pop(0)
                #remainingDuration.pop(light) #Can't change dict size during iteration?? Annoying... TODO use full schedules here
                if len(remainingDuration[light]) == 0:
                    break #Avoid errors - schedule doesn't go out this far
            if len(remainingDuration[light]) == 0:
                    break #Avoid errors - schedule doesn't go out this far
