...
def doSurtracThread(network, simtime, light, clusters, lightinfo, inQueueSim, predictionCutoff, toSwitch, catpreds, remainingDuration, bestschedules):
...
def doSurtrac(network, simtime, realclusters=None, lightinfo=None, predClusters=None):
    ...
    for light in lights:
        if multithreadSurtrac:
            surtracThreads[light] = threading.Thread(target=doSurtracThread, args=(network, simtime, light, clusters, lightinfo, inQueueSim, predictionCutoff, toSwitch, catpreds, remainingDuration, bestschedules))
            surtracThreads[light].start()
        else:
            doSurtracThread(network, simtime, light, clusters, lightinfo, inQueueSim, predictionCutoff, toSwitch, catpreds, remainingDuration, bestschedules)
        ...
        if not bestschedule[0] == []:
            ...
            if len(remainingDuration[light]) > 0:
                ...
                if remainingDuration[light][0] <= 0: #Light needs to change
                    ...
                    curphase = lightinfo[light]["index"]
                    nPhases = len(surtracdata[light]) #Number of phases
                    lightinfo[light]["index"] = (curphase+1)%nPhases

                    if len(remainingDuration[light]) == 0:
                        remainingDuration[light] = [lightphasedata[light][(lightinfo[light]["index"]+1)%len(lightphasedata[light])].duration]
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if simtime%100 == 0 or not traci.simulation.getMinExpectedNumber() > 0:
            ...
            for id in endDict:
                ...
                avgTime2 += ttemp2/len(endDict)
                if isSmart[id]:
                    avgTimeSmart2 += ttemp2/nSmart
                else:
                    avgTimeNot2 += ttemp2/(len(endDict)-nSmart)
...
def runClusters(net, routesimtime, mainRemainingDuration, vehicleOfInterest, startedge, loaddata, routes):
    ...
    #for light in lights:
        # print(light)
        # print(remainingDuration[light][0])
        # print(lightinfo[light]["switchtime"] - routesimtime)
        #assert(np.abs(remainingDuration[light][0] - (lightinfo[light]["switchtime"] - routesimtime)) < 1) #Sometimes fails with Surtrac. Consistent rRoundoff error around fourth decimal point. Does SUMO not store past that?
    ...
    while True:
        ...
        if routesimtime%surtracFreq >= (routesimtime+timestep)%surtracFreq:
            (toUpdate, queueSimPredClusters, newRemainingDuration) = doSurtrac(net, routesimtime, clusters, lightinfo, queueSimPredClusters)
            remainingDuration.update(newRemainingDuration)
        
        #lightinfo[light]["index"] should be getting updated in doSurtrac for things that change immediately; this shouldn't happen here
        #Switch phase on any light that needs to
        ...
        #     #This last part shouldn't matter anymore?
        ...
        for light in lights:
            if light not in remainingDuration or len(remainingDuration[light]) == 0:
                ...
                remainingDuration[light] = [lightphasedata[light][lightinfo[light]["index"]].duration]
            ...
            if remainingDuration[light][0] <= 0: #Note: Duration might not be divisible by timestep, so we might be getting off by a little over multiple phases
                ...
                lightinfo[light]["index"] = (lightinfo[light]["index"]+1)%len(lightphasedata[light])
                if len(remainingDuration[light]) == 0:
                    remainingDuration[light] = [lightphasedata[light][(lightinfo[light]["index"]+1)%len(lightphasedata[light])].duration] #+1 because we haven't changed phase yet
                    ...
                    while cartuple[1] < routesimtime:
                        ...
                        for nextlane in tempnextedges:
                            ...
                            if node in lights:
                                ...
                                for linktuple in prioritygreenlightlinks[node][lightinfo[node]["index"]]:
                                ...
                                if not isGreenLight: #No priority streams work; need to check non-priority streams
                                    if lane in lowprioritygreenlightlinksLE[node][lightinfo[node]["index"]] and nextedge in lowprioritygreenlightlinksLE[node][lightinfo[node]["index"]][lane]:
                                        for linktuple in lowprioritygreenlightlinksLE[node][lightinfo[node]["index"]][lane][nextedge]:
                                            #Make sure we're not a g stream blocked by a G stream
                                            isBlocked = False

                                            #Check if anything we've already sent through will block this
                                            for linktuple2 in prioritygreenlightlinks[node][lightinfo[node]["index"]]+lowprioritygreenlightlinks[node][lightinfo[node]["index"]]:
                                            ...
                                            if not isBlocked:
                                                for linktuple2 in prioritygreenlightlinks[node][lightinfo[node]["index"]]:
                                                    conflicting = lightlinkconflicts[node][linktuple][linktuple2] #Precomputed to save time
                                                    if not conflicting:
                                                        continue
...
def dontBreakEverything():
    ...
    print("Can comment the call to dontBreakEverything here - there's only one simulator")
...
def main(sumoconfig, pSmart, verbose = True):
    ...
    global lightlanes
    global notlightlanes
    global notlightoutlanes
    global lights
    global edges
    ...
    for light in lights:
        ...
        for i in range(n):
            ...
            surtracdata[light][i]["minDur"] = 5#lightphasedata[light][i].minDur
    ...
    # get root element
    root = tree.getroot() #Route root? Punny...

    actualStartDict = dict()
    # iterate news items
    for item in root.findall('./vehicle'):
        actualStartDict[item.attrib["id"]] = int(item.attrib["depart"])
