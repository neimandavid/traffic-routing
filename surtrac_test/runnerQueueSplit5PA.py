...
#@profile
def doSurtrac(network, simtime):
    (clusters, lightinfo) = loadClusters(network)
    ...
    for light in lights:
        ...
        emptyStatus = [None]*nPhases
        fullStatus = [None]*nPhases
        nClusters = 0
        for i in range(nPhases):
            nLanes = len(surtracdata[light][i]["lanes"])
            emptyStatus[i] = [0]*nLanes
            fullStatus[i] = [0]*nLanes
            for j in range(nLanes):
                fullStatus[i][j] = len(clusters[surtracdata[light][i]["lanes"][j]])
                nClusters = nClusters + fullStatus[i][j]
        ...
        schedules = [([], emptyStatus, phase, [simtime]*len(surtracdata[light][phase]["lanes"]), simtime, 0, lastSwitch, [simtime - lastSwitch])]
        
        
        #lightinfo[light]["switchtime"]
        for iternum in range(nClusters): #Keep adding a cluster until #clusters added = #clusters to be added
            ...
            for schedule in schedules:
                for i in range(nPhases):
                    nLanes = len(surtracdata[light][i]["lanes"])
                    for j in range(nLanes):
                        if schedule[1][i][j] == fullStatus[i][j]:
                            #No more clusters to add here, don't even try
                            continue
                        ...
                        newScheduleStatus[i][j] = newScheduleStatus[i][j] + 1
                        phase = schedule[2]

                        newDurations = pickle.loads(pickle.dumps(schedule[7])) #deepcopy(schedule[7])

                        clusterind = newScheduleStatus[i][j]-1
                        ist = clusters[surtracdata[light][i]["lanes"][j]][clusterind]["arrival"] #Intended start time = cluster arrival time
                        dur = clusters[surtracdata[light][i]["lanes"][j]][clusterind]["departure"] - ist
                        ...
                        if not phase == i:
                            ...
                            newDurations.append(0) #Duration of new phase (i). To be updated once we figure out when the cluster finishes
                        else:
                            pst = schedule[3][j] #Relevant directional makespan
                        ...
                        ast = max(ist, pst)
                        directionalMakespans[j] = ast+dur
                        delay = schedule[5] + len(clusters[surtracdata[light][i]["lanes"][j]][clusterind]["cars"])*(ast-ist) #Delay += #cars * (actual-desired)
                        newMakespan = max(directionalMakespans)
                        currentDuration = newMakespan - newLastSwitch 
                        if currentDuration > surtracdata[light][i]["maxDur"]:
                            #TODO: Should consider splitting the cluster at the max duration value
                            #Instead, I'll consider cycling through all phases and coming back (in case there's nothing in other directions)
                            #Copy the phase-switch logic
                            newFirstSwitch = max(schedule[6] + surtracdata[light][phase]["minDur"], schedule[4])
                            pst = newFirstSwitch + surtracdata[light][(phase+1)%nPhases]["timeTo"][i] + sult #Total makespan + switching time + startup loss time
                            #TODO: Technically this sult implementation isn't quite right, as a cluster might reach the light as the light turns green and not have to stop and restart
                            directionalMakespans = [pst]*nLanes #Other directions can't schedule a cluster before the light switches
                        ...
                        newschedule = (schedule[0]+[(i, j)], newScheduleStatus, i, directionalMakespans, newMakespan, delay, newLastSwitch, newDurations)
        ...
        if not bestschedule[0] == []:
            if light == "J16":
                print("Next switch time before update: ", traci.trafficlight.getNextSwitch(light))
                print("Phase before update: ", traci.trafficlight.getPhase(light))
            remainingDuration = traci.trafficlight.getNextSwitch(light) - simtime
            spentDuration = traci.trafficlight.getPhaseDuration(light) - remainingDuration #NEXT TODO: This may not be what I want, because getPhaseDuration nonsense...
            spentDuration = simtime - lastswitchtimes[light]
            #How to fix: Maintain a dictionary of when the last switch actually was
            #Use that up top when computing when I can switch again
            #I compute a total duration for the phase
            #spentDuration = now - last switch (instead of now - (next switch - duration) because duration isn't getting maintained properly)
            #Should be good from there as long as I update the dictionary below if the phase changes

            if bestschedule[7][0] - spentDuration > 0:
                #Update duration
                ...
                if light == "J16": print("Setting duration to ", bestschedule[7][0] - spentDuration)
            else:
                ...
                if len(bestschedule[7]) > 1:
                    ...
                    #pass
                if light == "J16": print("Light should switch")

            #traci.trafficlight.setPhaseDuration(light, bestschedule[7][0]) 
            if light == "J16":
                print("Next switch time after update: ", traci.trafficlight.getNextSwitch(light)) #Does update
                print("Phase after update: ", traci.trafficlight.getPhase(light))
                print("Simulation time after update: ", simtime)
            #print(traci.trafficlight.getPhaseDuration(light)) #DOES NOT GET UPDATED by setPhaseDuration, just reads off the preprogrammed duration value


        #break #Hack to only update one light
...
def run(network, rerouters, pSmart, verbose = True):
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        simtime += 1
        # for light in lights:
        #     if traci.trafficlight.getNextSwitch(light) == simtime:
        #         #lastswitchtimes[light] = simtime #This apparently makes the light get stuck??
        #         pass
        traci.simulationStep() #Tell the simulator to simulate the next time step
        doSurtrac(network, simtime)
        ...
        for vehicle in traci.simulation.getDepartedIDList():
            ...
            #traci.vehicle.setRoute(vehicle, traci.simulation.findRoute(route[0], route[-1]).edges)
...
def QueueReroute(detector, network, rerouteAuto=True):
    ...
    for vehicle in ids:
        ...
        # #This shouldn't go off, since we do this when the vehicle enters the network
        ...
        if rerouteAuto and isSmart[vehicle]:
            ...
            #TODO: Should be doing nothing here, just testing for now
...
def runClusters(net, time, vehicleOfInterest, startedge, loaddata):
    ...
    for vehicle in vehicles:
        if isSmart[vehicle]:
            ...
            routes[vehicle] = sampleRouteFromTurnData(vehicle, traci.vehicle.getRoadID(vehicle), turndata)
...
def sampleRouteFromTurnData(vehicle, startedge, turndata):
    edge = startedge
    route = [edge]
    while edge in turndata:
        #edge = randomSampleFromTurnData
        r = random.random()
        for nextedge in turndata[edge]:
            r -= turndata[edge][nextedge]
            if r <= 0:
                edge = nextedge
                break
        route.append(edge)
...
def main(sumoconfig, pSmart, verbose = True):
    ...
    for light in lights:
        ...
        #print(traci.trafficlight.getCompleteRedYellowGreenDefinition(light))
        #print(traci.trafficlight.getControlledLinks(light))
        #Has lagging lefts
        ...
        for i in range(n):
            ...
            for linklistind in range(len(linklistlist)):
                ...
                if linkstate == "G" and not linklistlist[linklistind][0][0] in surtracdata[light][i]["lanes"]: #NOTE: I'm being sloppy and assuming one-element lists of tuples, but I've yet to see a multi-element list here
                    surtracdata[light][i]["lanes"].append(linklistlist[linklistind][0][0])
    ...
    if pSmart < 1:
        with open("turndata_"+routefile.split(".")[0]+".pickle", 'rb') as handle:
