...
timestep = 2.5 #mingap#1#mingap #Amount of time between updates. In practice, mingap rounds up to the nearest multiple of this
...
reuseSurtrac = False #Does Surtrac computations in a separate thread, shared between all vehicles doing routing. Keep this true unless we need everything single-threaded (ex: for debugging)
debugMode = True #Enables some sanity checks and assert statements that are somewhat slow but helpful for debugging
mainSurtracFreq = 1e6 #Recompute Surtrac schedules every this many seconds in the main simulation (technically a period not a frequency)
routingSurtracFreq = 1e6 #Recompute Surtrac schedules every this many seconds in the main simulation (technically a period not a frequency)
recomputeRoutingSurtracFreq = 5 #Maintain the previously-computed Surtrac schedules for all vehicles routing less than this many seconds in the main simulation. Set to 1 to only reuse results within the same timestep. Does nothing when reuseSurtrac is False.
...
#Test durations to see if there's drift between fixed timing plans executed in main simulation and routing simulations.
#TODO: What exactly did this do, again?
simdurations = dict()
            ...
            for lane in lightlanes[light]:
                ...
                for i in lanephases[lane]:
                    ...
                    if disableSurtracPred:
                        ...
                        newPredClusters = pickle.loads(pickle.dumps(schedule[8])) #Deep copy needed if I'm going to merge clusters
                        predLanes = []
                        ...
                        for cartuple in clusters[lane][clusterind]["cars"]:
                            ...
                            if not cartuple[0] in isSmart or isSmart[cartuple[0]]: #It's possible we call this from QueueSim, at which point we split the vehicle being routed and wouldn't recognize the new names. Anything else should get assigned to isSmart or not on creation
                                ...
                                #NEXT TODO: Can't I just precompute all of these probabilities??
                                # normprob = 0
                                ...
                                # print(turndata[lane])
                                # print(normprob)
                                # print(normprobs[lane][nextedge])
                                # assert(abs(normprob - normprobs[lane][nextedge]) < 1e-10)
                                # if normprob == 0:
...
def doSurtrac(network, simtime, realclusters=None, lightphases=None, lastswitchtimes=None, predClusters=None):
    ...
    if realclusters == None and lightphases == None:
        ...
        if clustersCache == None:
            clustersCache = loadClusters(network)
        ...
        if not bestschedule[0] == []:
            ...
            if len(remainingDuration[light]) > 0:
                ...
                if remainingDuration[light][0] <= 0: #Light needs to change
                    ...
                    remainingDuration[light].pop(0) #RECENT CHANGE: Pretty sure we have to pop the queue before checking if the queue is empty after popping...
        ...
        for id in laneDict:
            ...
            if newlane != laneDict[id] and len(newlane) > 0 and  newlane[0] != ":":
                ...
                if newlane.split("_")[0] == currentRoutes[id][-1]:
                    #print("Computing distance for " + id)
...
def QueueReroute(detector, network, reroutedata, simtime, remainingDuration):
    ...
    for vehicle in ids:
        ...
        if isSmart[vehicle]:
            ...
            if clustersCache == None:
                clustersCache = loadClusters(network, vehicle)
            ...
            if reuseSurtrac:
                if killSurtracThread:
                ...
                #if nToReroute == 1:
                    #print("Starting new Surtrac thread: Time: " + str(simtime) + " , vehicle: " + vehicle)
                    ...
                    #print("Reusing old Surtrac results as planned. Time: " + str(simtime) + " , vehicle: " + vehicle)
                    pass
...
def runClusters(net, routesimtime, mainRemainingDuration, vehicleOfInterest, startlane, loaddata, routes):
    ...
    if not computeSurtrac:
        ...
        for lanenum in range(0, lanenums[startedge]):
            ...
            VOIs.append(vehicle)
            for cluster in clusters[lane]: #Loop over all existing clusters in the lane, looking for a reasonable one to append to
                ...
                if cluster["startpos"] >= startdist and cluster["endpos"] <= startdist:
                    ...
                    #assert(cluster["time"] == routesimtime) #Might not hold now that we're messing with initial routesimtime to keep Surtrac call times consistent. Should be close though, and shouldn't cause problems.
    ...
    while True:
        ...
        #Round down to a multiple of the timestep so sims starting at different times ask for light info at fixed times.
        #NEXT TODO: Does this avoid breaking stuff
        #routesimtime = math.floor(routesimtime/timestep)*timestep
