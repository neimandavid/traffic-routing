...
def run(netfile, rerouters):
    ...
    print("Total run time: " + str(time.time() - tstart))
...
def doClusterSim(prevedge, net, vehicle):
    ...
    while route == "NOT DONE":
        route = stepClusters(net, simtime, vehicle, prevedge)
        simtime+=timestep
    return (route, simtime-starttime)
                            ...
                            for nextedge in nextedges:
                                for nextlanenum in range(traci.edge.getLaneNumber(nextedge)):
                                    ...
                                    if not cartuple[0] in VOIs:
                                        ...
                                        if routeind + 2 < len(route): #If no next next edge, don't be picky
